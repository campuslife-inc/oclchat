...
<body>
	<div class="container">
		...
		foreach($_SESSION['user_data'] as $key => $value){
			...
			<img src="<?=$value['profile']?>" class="img-fluid rounded-circle img-thumbnail" width="150"/>
		...
		foreach($user_data as $key => $user)
		{
			...
			if($user['user_id'] != $login_user_id)
			{
				...
				<span id='userid_".$user['user_id']."'></span>".$total_unread_message."</strong>
				</span>
		...
		<br/>
		<h3 class="text-center">Realtime one to one chat app using ratchet websockets with php mysql - design chat area - 4</h3>
		<hr/>
		<br/>
		<div id="chat_area">
		</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		{
			var html='<div class="card"><div class="card-header"><div class="row"><div class="col col-sm-6"><b>chat with <span class="text-danger" id="chat_user_name">'+user_name+'</span></b></div><div class="col col-sm-6 text-right"> <a href="chatroom.php" class="btn btn-success btn-sm">Group Chat</a> <button type="button" class="close" id="close_chat_area" data-dismiss="alert" area-label="Close"><span area-hidden="true">&times;</span></button> </div></div></div><div class="card-body" id="message_area"></div></div><form id="chat_form" method="POST" data-parsley-errors-container="#validation_error"><div class="input-group mb-3" style="height:7vh"><textarea class="form-control" id="chat_message" name="chat_message" placeholder="Type Message Here" data-parsley-maxlength="1000" data-parsley-pattern="/^[a-zA-Z0-9]+$/" required></textarea><div class="input-group-append"><button type="submit" name="send" id="send" class="btn btn-primary"><i class="fa fa-paper-plane"></i></button></div></div><div id="validation_error"></div><br/></form>';
		...
		$(document).on('click','.select_user',function(){
			...
			$(document).on('submit','#chat_form',function(event){
				...
				url:"action.php",
				method:"POST",
				data:{action:'save_chat',user_id:user_id,msg: message,receiver_userid:receiver_userid,datetime:datetime,},
				dataType:"text",
				success:function(data)
				{
					alert(data);
